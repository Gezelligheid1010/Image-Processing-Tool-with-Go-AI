...
<!--      <div class="form-btn">-->
        <button type="button" class="custom-button" @click="goToSignUp">没有账号？去注册</button>
<!--      </div>-->
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    submit() {
      this.$axios({
        ...
        if (res.code == 1000) {
          ...
          // console.log('loginResult:',localStorage.getItem('loginResult'))
    ...
    goToSignUp() {
      this.$router.push({ name: "SignUp" });  // 跳转到注册页面
    }
  }
};
</script>
...
.main {
  ...
  .container {
    ...
    .custom-button {
      ...
      padding: 8px 16px; /* 可根据需要调整 */
    }

    .custom-button:hover {
      color: darkblue; /* 悬浮时字体变深 */
